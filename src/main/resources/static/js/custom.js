...
function ajaxFormValidationSubmit(event) {
	
		var $form = $(this);
		// Ajax validation
		var $inputs = $form.find('input');
		var data = collectFormData($inputs);
		var processedValidateUrl = $form.find("#ajaxValidationUrl").val();
		
		$.post(processedValidateUrl, data, function(response) {
			$form.find('.control-group').removeClass('has-error');
			$form.find('.help-block').empty();
			$form.find('.alert').remove();
			$form.find('.global-errors').empty();
			
			if (response.status == 'FAIL') {
				for (var i = 0; i < response.errorMessageList.length; i++) {
					var item = response.errorMessageList[i];
					var $controlGroup = $form.find('#' + item.fieldName + '_ctr_grp');
...
$(document).ready(function($){
	...
	$(".resubmitFormSubmitBtn" ).bind( "click", function(event) {
		var $btn = $(this);
		var value = $btn.closest('tr').find("input[id='changeQTy']").val();
		if (!value.match(/^\d+$/)) {
